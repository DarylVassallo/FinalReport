...
public class OVRCameraRig : MonoBehaviour
{
    ...
    //Shows The Amplification Value Used To Amplify The Level Size And Player Movements
    public float amplifier;

    //Shows If The Player's Movements Can Be Amplified
    public bool canAmplify;

    //Shows If The Origin Point Can be Changed
    public bool changeOriginPos;

    //Changes The Player's Starting Point
    private Vector3 spawnPosDiff;

    //Changes The Orign Point So That The Amplified Position Starts From The Player's Current Position
    private Vector3 originPointDiff;
    ...
    protected virtual void UpdateAnchors(bool updateEyeAnchors, bool updateHandAnchors)
    {
        ...
        if (updateEyeAnchors)
        {
            if (hmdPresent)
            {
                ...
                {
                    //Sets The Player's Starting Position To The Center, And Shows That The Player Is Being Tracked:
                    //If The Player Is Not Shown To Be Tracked, And If The Player's Position Is Not At The Origin Point
                    if (isTracking == false && centerEyePosition != new Vector3(0, 0, 0))
                    {
                        //Initialises The Player's Starting Point
                        Vector3 startingPoint = new Vector3(0, 0, 0);

                        //Finds The Player's Correct Starting Point Using The 'cornerData.text' File:
                        //If The Current Level Is Not The Introduction Or Set Up Level
                        if (duringSetUp == false)
                        {
                            ...
                            //Finds The Player's Correct Starting Point
                            string[] linePos = textLines[6].Split(char.Parse(","));
                            startingPoint = new Vector3(float.Parse(linePos[0]), 0, float.Parse(linePos[2]));
                        }

                        //Sets The Player To The Starting Point And Shows That The Player Is Currently Being Tracked
                        spawnPosDiff = startingPoint - centerEyePosition;
                        isTracking = true;
                    }

                    //Changes The Orign Point For The Player, While Keeping Them In that Position,
                    //And Shows That The Origin Point Cannot Be Changed:
                    //If It Is Shown That The Orign Point Can Be Changed
                    if(changeOriginPos == true)
                    {
                        originPointDiff = centerEyeAnchor.localPosition;

                        spawnPosDiff = -centerEyePosition;
                        changeOriginPos = false;
                    }
                }

                //Checks If The Player's Movements Need To Be Amplified
                if (canAmplify == true) {
                    if (OVRNodeStateProperties.GetNodeStatePropertyVector3(Node.CenterEye, NodeStatePropertyType.Position, OVRPlugin.Node.EyeCenter, OVRPlugin.Step.Render, out centerEyePosition))
                    {
                        //The Player's Position Is Modified To Change The Player's Starting Point 
                        centerEyePosition = new Vector3(centerEyePosition.x + spawnPosDiff.x, centerEyePosition.y, centerEyePosition.z + spawnPosDiff.z);


                        //The Player's Position Is Amplified, And The New Position Is Changed To Move The Player To The Correct Position
                        centerEyeAnchor.localPosition = new Vector3((   centerEyePosition.x * amplifier) + originPointDiff.x,
                                                                        centerEyePosition.y,
                                                                       (centerEyePosition.z * amplifier) + originPointDiff.z);
                    }
                }else{
                    if (OVRNodeStateProperties.GetNodeStatePropertyVector3(Node.CenterEye, NodeStatePropertyType.Position, OVRPlugin.Node.EyeCenter, OVRPlugin.Step.Render, out centerEyePosition))
                    {
                        ...
                        //The Player's Position Is Changed To Move The Player To The Correct Position
                        centerEyeAnchor.localPosition = new Vector3(centerEyePosition.x + originPointDiff.x,
                                                                    centerEyePosition.y,
                                                                    centerEyePosition.z + originPointDiff.z);
                    }
                }
            ...
            else
            {
                ...
                if (canAmplify == true)
                {
                    ...
                    if (OVRNodeStateProperties.GetNodeStatePropertyVector3(Node.CenterEye, NodeStatePropertyType.Position, OVRPlugin.Node.EyeCenter, OVRPlugin.Step.Render, out centerEyePosition))
                    {
                        ...
                        //The Player's Hand Positions Are Amplified, And The New Positions Are Changed To Move The Hands To Their Correct Position
                        leftHandAnchor.localPosition = new Vector3((OVRInput.GetLocalControllerPosition(OVRInput.Controller.LTouch).x + ((centerEyePosition.x * amplifier) - centerEyePosition.x + spawnPosDiff.x)) + originPointDiff.x,
                                                                    OVRInput.GetLocalControllerPosition(OVRInput.Controller.LTouch).y,
                                                                    (OVRInput.GetLocalControllerPosition(OVRInput.Controller.LTouch).z + ((centerEyePosition.z * amplifier) - centerEyePosition.z + spawnPosDiff.z)) + originPointDiff.z);

                        rightHandAnchor.localPosition = new Vector3((OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch).x + ((centerEyePosition.x * amplifier) - centerEyePosition.x + spawnPosDiff.x)) + originPointDiff.x,
                                                                    OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch).y,
                                                                    (OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch).z + ((centerEyePosition.z * amplifier) - centerEyePosition.z + spawnPosDiff.z)) + originPointDiff.z);
                    }
                }
                else
                {
                    //The Player's Hand Positions Are Changed To Move The Hands To Their Correct Position
                    leftHandAnchor.localPosition = new Vector3( (OVRInput.GetLocalControllerPosition(OVRInput.Controller.LTouch).x + spawnPosDiff.x) + originPointDiff.x,
                                                                OVRInput.GetLocalControllerPosition(OVRInput.Controller.LTouch).y,
                                                                (OVRInput.GetLocalControllerPosition(OVRInput.Controller.LTouch).z + spawnPosDiff.z) + originPointDiff.z);

                    rightHandAnchor.localPosition = new Vector3((OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch).x + spawnPosDiff.x) + originPointDiff.x,
                                                                OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch).y,
                                                                (OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch).z + spawnPosDiff.z) + originPointDiff.z);
                }
